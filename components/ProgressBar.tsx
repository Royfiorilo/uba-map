...
export default function ProgressBar() {
  // Ahora usamos statusByCareer como única fuente de la verdad
  const { currentCareer, statusByCareer, notasByCareer } = useCareerStore();
  ...
  const segmentos = useMemo(() => {
    ...
    return Array.from(ciclosMap.entries()).map(([cicloNombre, materias]) => {
      ...
      // SOLUCIÓN: Calculamos si está aprobada leyendo directo del diccionario nuevo
      const aprobadas = materias.filter((m: any) => materiaStatuses[m.id] === 'aprobada').length;
  ...
  }, [carrera, materiaStatuses]);

  const promedio = useMemo(() => {
    let sum = 0;
    let count = 0;

    // SOLUCIÓN: Calculamos el promedio iterando las materias reales de esta carrera
    carrera.nodos.forEach((m: any) => {
      if (m.id !== 'cbc_master' && materiaStatuses[m.id] === 'aprobada') {
        const nota = materiaNotas[m.id] || 4; 
        sum += nota;
        count++;
      }
    });
  ...
  }, [materiaStatuses, materiaNotas, carrera]);
  ...
  return (
    <div className="w-full bg-zinc-950/90 backdrop-blur-md border-t border-white/10 px-6 py-3 flex flex-col gap-3">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex flex-wrap gap-6 flex-1 items-end">
          {segmentos.map((s) => (
            <div key={s.label} className="flex flex-col">
              <span className="text-[9px] font-black text-zinc-500 uppercase tracking-tighter truncate max-w-[80px]" title={s.label}>
                {s.label}
              </span>
        ...
        <div className="flex flex-col items-end pl-6 border-l border-zinc-800 ml-4">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Promedio</span>
          </div>
      ...
      <div className="flex h-5 w-full bg-zinc-900 rounded-md overflow-hidden border border-white/5 shadow-inner relative">
        {segmentos.map((s) => (
          <div 
            key={s.label}
